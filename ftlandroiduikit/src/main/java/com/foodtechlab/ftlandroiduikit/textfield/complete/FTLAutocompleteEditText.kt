...
import com.foodtechlab.ftlandroiduikit.textfield.complete.FTLCompleteEditTextThemeManager
...
class FTLAutocompleteEditText @JvmOverloads constructor(
    ...
    private val viewThemeManager: ViewThemeManager<FTLCompleteEditTextTheme> =
        FTLCompleteEditTextThemeManager()
    private val job = SupervisorJob()
    override val coroutineContext: CoroutineContext
        get() = Dispatchers.Main + job
    private var ftlAutocompleteEditTextTheme = viewThemeManager.lightTheme
    ...
    private val controlsColor: Int
        get() = ContextCompat.getColor(
            context, when {
                isErrorEnabled -> ftlAutocompleteEditTextTheme.errorControlColor
                !etInput.hasFocus() && !etInput.text.isNullOrEmpty() -> ftlAutocompleteEditTextTheme.defaultControlColor
                isHintOnTop -> ftlAutocompleteEditTextTheme.activeControlColor
                else -> ftlAutocompleteEditTextTheme.defaultControlColor
            }
        )
    ...
    private fun onThemeChanged(theme: FTLCompleteEditTextTheme) {
        ...
        with(etInput) {
            ...
            setDropDownBackgroundResource(theme.dropDownBackgroundDrawable)
        }

        DrawableCompat.setTint(
            ivClearText.drawable,
            ContextCompat.getColor(context, theme.clearIconTintColor)
        )
